...
async function handleSimpanNilai(e) {
    ...
    const dataForm = {
        ...
        Tanggal_Penilaian: new Date().toISOString().slice(0, 10), // Set tanggal hari ini
        Jenis_Nilai: 'Karakter',
        Aspek_Yang_Dinilai: document.getElementById('aspekDinilai').value,
        Nilai_Deskriptif: document.getElementById('nilaiDeskriptif').value,
        Umpan_Balik_Siswa: document.getElementById('umpanBalik').value
    };

    if (!dataForm.ID_Siswa || !dataForm.Aspek_Yang_Dinilai) {
        tampilkanNotifikasi('Siswa dan Aspek yang Dinilai wajib diisi.', 'error');
